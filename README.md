...
```mermaid
flowchart LR
  themePkg["@kareem-hany/theme (npm)"]
...
| Piece | What it is |
|-------|-----------|
| [`@kareem-hany/theme`](./packages/theme) | An npm package with the shared design tokens: light/dark CSS variables, fonts, and the signature blob border-radius system. |
...
```css
@import "@kareem-hany/theme/fonts.css";  /* first, so the font @import is valid */
@import "tailwindcss";
...
doodle-ui/
  ...
  packages/theme/      # @kareem-hany/theme (published to npm)
