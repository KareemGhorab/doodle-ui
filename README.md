...
```mermaid
flowchart LR
  themePkg["@kareem-ghorab/theme (npm)"]
...
| Piece | What it is |
|-------|-----------|
| [`@kareem-ghorab/theme`](./packages/theme) | An npm package with the shared design tokens: light/dark CSS variables, fonts, and the signature blob border-radius system. |
...
```css
@import "@kareem-ghorab/theme/fonts.css";  /* first, so the font @import is valid */
@import "tailwindcss";
...
doodle-ui/
  ...
  packages/theme/      # @kareem-ghorab/theme (published to npm)
